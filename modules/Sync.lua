...
function Sync.SendHeartbeat()
    ...
    if Config.IsDebugMode() then
        ...
        print(string.format("|cff00ff00[GuildWorkOrders Debug]|r Broadcasting heartbeat: order %s v%d (%s) - %s [%d/%d]", 
            orderToSend.id, orderToSend.version or 1, orderToSend.status, 
            orderToSend.itemName or "Unknown Item", heartbeatIndex, #ordersToSend))
    end
